...
void Debouncer_Start(void) 
{
    ...
    Debouncer_CLKEN |= Debouncer_CLKEN_MASK;
	Debouncer_CLKSTBY |= Debouncer_CLKSTBY_MASK;
}
...
void Debouncer_Stop(void) 
{
    ...
    Debouncer_CLKEN &= (uint8)(~Debouncer_CLKEN_MASK);
	Debouncer_CLKSTBY &= (uint8)(~Debouncer_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Debouncer_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Debouncer_CLKEN & Debouncer_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Debouncer_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Debouncer_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Debouncer__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Debouncer_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Debouncer_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Debouncer_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Debouncer_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Debouncer_GetSourceRegister();
    uint16 oldDivider = Debouncer_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Debouncer_CLKEN & Debouncer_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Debouncer_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Debouncer_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Debouncer_DIV_PTR);
}


/*******************************************************************************
* Function Name: Debouncer_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Debouncer_MOD_SRC |= modeBitMask & (uint8)Debouncer_MODE_MASK;
}


/*******************************************************************************
* Function Name: Debouncer_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Debouncer_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Debouncer_MODE_MASK));
}


/*******************************************************************************
* Function Name: Debouncer_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Debouncer_MOD_SRC & (uint8)(Debouncer_MODE_MASK);
}


/*******************************************************************************
* Function Name: Debouncer_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Debouncer_GetDividerRegister();
    uint8 oldSrc = Debouncer_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Debouncer_MOD_SRC |= CYCLK_SSS;
        Debouncer_MOD_SRC =
            (Debouncer_MOD_SRC & (uint8)(~Debouncer_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Debouncer_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Debouncer_MOD_SRC & Debouncer_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Debouncer_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Debouncer_PHASE = clkPhase & Debouncer_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Debouncer_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Debouncer_PHASE & Debouncer_PHASE_MASK;
}
