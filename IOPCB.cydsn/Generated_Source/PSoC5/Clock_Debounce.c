...
/*******************************************************************************
* Function Name: Clock_Debounce_Start
********************************************************************************
*
* Summary:
...
void Clock_Debounce_Start(void) 
{
    ...
    Clock_Debounce_CLKEN |= Clock_Debounce_CLKEN_MASK;
	Clock_Debounce_CLKSTBY |= Clock_Debounce_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Debounce_Stop
********************************************************************************
*
* Summary:
...
void Clock_Debounce_Stop(void) 
{
    ...
    Clock_Debounce_CLKEN &= (uint8)(~Clock_Debounce_CLKEN_MASK);
	Clock_Debounce_CLKSTBY &= (uint8)(~Clock_Debounce_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Debounce_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Debounce_CLKEN & Clock_Debounce_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Debounce_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Debounce_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Debounce__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Debounce_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Debounce_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Debounce_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Debounce_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Debounce_GetSourceRegister();
    uint16 oldDivider = Clock_Debounce_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Debounce_CLKEN & Clock_Debounce_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_Debounce_DIV_PTR, clkDivider);
                Clock_Debounce_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_Debounce_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_Debounce_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Debounce_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Debounce_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Debounce_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Debounce_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Debounce_MOD_SRC |= modeBitMask & (uint8)Clock_Debounce_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Debounce_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Debounce_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Debounce_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Debounce_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Debounce_MOD_SRC & (uint8)(Clock_Debounce_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Debounce_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Debounce_GetDividerRegister();
    uint8 oldSrc = Clock_Debounce_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Debounce_MOD_SRC |= CYCLK_SSS;
        Clock_Debounce_MOD_SRC =
            (Clock_Debounce_MOD_SRC & (uint8)(~Clock_Debounce_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Debounce_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Debounce_MOD_SRC & Clock_Debounce_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Debounce_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Debounce_PHASE = clkPhase & Clock_Debounce_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Debounce_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Debounce_PHASE & Clock_Debounce_PHASE_MASK;
}
