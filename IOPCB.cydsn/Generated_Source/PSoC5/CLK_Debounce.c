...
/*******************************************************************************
* Function Name: CLK_Debounce_Start
********************************************************************************
*
* Summary:
...
void CLK_Debounce_Start(void) 
{
    ...
    CLK_Debounce_CLKEN |= CLK_Debounce_CLKEN_MASK;
	CLK_Debounce_CLKSTBY |= CLK_Debounce_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CLK_Debounce_Stop
********************************************************************************
*
* Summary:
...
void CLK_Debounce_Stop(void) 
{
    ...
    CLK_Debounce_CLKEN &= (uint8)(~CLK_Debounce_CLKEN_MASK);
	CLK_Debounce_CLKSTBY &= (uint8)(~CLK_Debounce_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLK_Debounce_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CLK_Debounce_CLKEN & CLK_Debounce_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CLK_Debounce_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLK_Debounce_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CLK_Debounce__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CLK_Debounce_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLK_Debounce_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLK_Debounce_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CLK_Debounce_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CLK_Debounce_GetSourceRegister();
    uint16 oldDivider = CLK_Debounce_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLK_Debounce_CLKEN & CLK_Debounce_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CLK_Debounce_DIV_PTR, clkDivider);
                CLK_Debounce_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CLK_Debounce_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CLK_Debounce_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLK_Debounce_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CLK_Debounce_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CLK_Debounce_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLK_Debounce_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_Debounce_MOD_SRC |= modeBitMask & (uint8)CLK_Debounce_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLK_Debounce_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_Debounce_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLK_Debounce_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLK_Debounce_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_Debounce_MOD_SRC & (uint8)(CLK_Debounce_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLK_Debounce_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CLK_Debounce_GetDividerRegister();
    uint8 oldSrc = CLK_Debounce_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLK_Debounce_MOD_SRC |= CYCLK_SSS;
        CLK_Debounce_MOD_SRC =
            (CLK_Debounce_MOD_SRC & (uint8)(~CLK_Debounce_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLK_Debounce_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_Debounce_MOD_SRC & CLK_Debounce_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLK_Debounce_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CLK_Debounce_PHASE = clkPhase & CLK_Debounce_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLK_Debounce_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CLK_Debounce_PHASE & CLK_Debounce_PHASE_MASK;
}
